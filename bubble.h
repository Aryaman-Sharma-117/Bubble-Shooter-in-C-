...
const int BUBBLE_DEFAULT_ACC = 10;
...
public:
    ...
    {
        // acc introduced to get a parabolic path for bubbles
        vy += BUBBLE_DEFAULT_ACC*t;
        ...
        if ((vy > 0 && new_y > (450 - circle.getRadius())) // bounce along Y direction at lower border
            ||
            (vy < 0 && new_y < (0 + circle.getRadius())))  // bounce along Y direction at upper border
        {
